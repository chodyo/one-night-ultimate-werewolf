...
export class MyRoom extends Room {
    ...
    ready(client: Client, params: any): void {
        ...
        messages.forEach((message: string, player: Player) => {
            ...
            const playerName = player.name === null ? player.sessionId : player.name;
            console.debug(
                `Notifying ${playerName} of their ${phaseAction} choice as ${roleID} with custom message "${message}"`
            );
            const client = this.clients.filter((client) => client.sessionId === player.sessionId)[0];
            this.messager.Notify(client, message, roleID);
        });
    }
